...
const MessagePusherComponent: React.FC = () => {
    ...
    const getNewMessages = useCallback(() => {
        ...
        return sortedMessages.filter(msg => {
            ...
            return msgId > lastSentIdRef.current && !processedMessagesRef.current.has(msg.id);
        });
    }, [messages]);

    // Send messages to webhook
    const sendMessages = useCallback(async (messagesToSend: any[]) => {
        ...
        try {
            const maxId = Math.max(...messagesToSend.map(m =>
    ...
    useEffect(() => {
        ...
        const hasNewMessages = JSON.stringify(messages) !== JSON.stringify(lastMessagesRef.current);
        
        if (hasNewMessages) {
            // Set a new timeout to wait for messages to stabilize
            ...
            }, 1000); // Wait 1 second after the last message update
        }
